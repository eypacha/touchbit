...
(function (root, factory) {
  if (typeof define === "function" && define.amd) {
    ...
    root.lzma = factory();
    root.LZMA = root.lzma; // Mantener compatibilidad con código existente
  }
})(typeof self !== "undefined" ? self : this, function () {
  // Polyfill para Worker
  if (!Worker) {
  ...
  // Constructor de LZMA
  return function (lzma_path) {
    var action_compress = 1,
      action_decompress = 2,
      action_progress = 3,
      callback_obj = {},
      ///NOTE: Node.js needs something like "./" or "../" at the beginning.
      lzma_worker = new Worker(
        typeof lzma_path === "undefined" ? "./lzma_worker.js" : lzma_path
      );

    lzma_worker.onmessage = function (e) {
      if (e.data.action === action_progress) {
        if (
          callback_obj[e.data.callback_num] &&
          typeof callback_obj[e.data.callback_num].on_progress === "function"
        ) {
        ...
        if (
          callback_obj[e.data.callback_num] &&
          typeof callback_obj[e.data.callback_num].on_finish === "function"
        ) {
